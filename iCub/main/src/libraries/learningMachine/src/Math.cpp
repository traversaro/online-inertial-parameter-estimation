...
void choldecomp(const yarp::sig::Matrix& A, yarp::sig::Matrix& R) {
    ...
    info = gsl_linalg_cholesky_decomp(Rgsl);
...
yarp::sig::Matrix choldecomp(const yarp::sig::Matrix& A) {
    yarp::sig::Matrix R(A.rows(),A.cols());
    choldecomp(A,R);
    return R;
}
...
unsigned int rankSVD(const yarp::sig::Matrix& M, double tol) {
    yarp::sig::Matrix U,V;
    yarp::sig::Vector S;
    unsigned int i;
    
    yarp::math::SVD(M,U,S,V);
    
    std::cerr << "rankSVD: called with condition number " << S[0]/S[S.size()-1] << std::endl;
    
    if( tol < 0 ) {
        tol = S[0]*std::numeric_limits<double>::epsilon()*std::max(M.rows(),M.cols());
    }
...
bool isfullrank(const yarp::sig::Matrix& M, double tol) {
    return ( rankSVD(M,tol) == std::min(M.cols(),M.rows()) );
}

yarp::sig::Vector diagonal(yarp::sig::Matrix & mat) {
    assert(mat.cols()==mat.rows());
    yarp::sig::Vector ret = yarp::sig::Vector(mat.cols());

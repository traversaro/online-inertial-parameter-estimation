...
/**
 * Computes the Cholesky factor R (in Gsl sense) of input matrix A
 * 
 * @param A the matrix A
 * @param R the Cholesky factor
 */
 void choldecomp(const yarp::sig::Matrix& A, yarp::sig::Matrix& R);
 
/**
 * Computes the Cholesky factor R (in Gsl sense) of input matrix A
 * 
 * @param A the matrix A
 * @return R the Cholesky factor
 */
 yarp::sig::Matrix choldecomp(const yarp::sig::Matrix& A);
...
/**
 * Computes the rank of a matrix. The routing uses the SVD on the
 * matrix, and then counts the non zero singular values. If provided, the
 * second parameter is the tolerance, otherwise the used tolerance is 
 * calculated using the standard heuristics tol = \sigma_1 * machine_epsilon *
 * 
 * @param M     a constant reference to the matrix
 * @param tol   the numerical tolerance 
 * @return      the rank of the matrix
 */
unsigned int rankSVD(const yarp::sig::Matrix& M, double tol = -1.0);

/**
 * Computes if a Matrix is of full rank. If provided, the
 * second parameter is the tolerance, otherwise the used tolerance is 
 * calculated using the standard heuristics tol = \sigma_1 * machine_epsilon *
 * 
 * @param M     a constant reference to the matrix
 * @param tol   the numerical tolerance 
 * @return      true if the Matrix is full rank, false otherwise
 */
bool isfullrank(const yarp::sig::Matrix& M, double tol = -1.0);

/**
 * Returns a vector containing the diagonal elements of the input matrix
 * 
 */
yarp::sig::Vector diagonal(yarp::sig::Matrix & mat);
